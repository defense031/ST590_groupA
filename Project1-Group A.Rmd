...
## Alternative Method to fix list columns (Placeholder)
```{r MoreUnnestingf}
...
# + coord_flip() can put the categories in y axis, which results in more space.
...
g2+geom_bar(aes(fill = as.factor(location_categories)))+coord_flip()

g3<-ggplot(distinct_summary_locations_interests, aes(x=location_categories, y=efficien_impressions))
g3+geom_boxplot()

#how do you guys feel about removing the NA's?  
#distinct_summary_locations_interests <- df %>% group_by(location_categories, interests_categories) %>% distinct(own_id, .keep_all = TRUE) %>% drop_na(location_categories) %>% drop_na(interests_categories) 

#the graph below is with NA removed from both variables

#getting this error:
#Error in order(y) : unimplemented type 'list' in 'orderVector1'
# error fixed, but there are some abnormal efficient impressions in unidentified location categories (NA) 
...
# Visual suggests there is an association between the two variables, not necessarily causation.  Further analysis and optimization could show how much variance of the conversion rate can be explaied by the number of ad click.
