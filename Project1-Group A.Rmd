...
## Numeric Summary of Average Spawns
```{r}
avgSpawnSummary = c()
typevector <- c("Normal","Grass","Flying","Poison","Electric","Ground","Fire","Water","Dragon","Ice","Ghost","Psychic","Rock","Bug","Fighting")
for (i in typevector){
  avgSpawnSummary <- c(avgSpawnSummary,(summary(as.numeric((filter(Pokedf,(Type1)%in%(i)|(Type2)%in%(i)))$avg_spawns))))}
...
##Find mean of spawn chance by type
```{r}
